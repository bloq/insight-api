...
var bcash = require('bitcore-lib-cash');
...
TxController.prototype.transformInput = function(options, input, index) {
  ...
  var addr;
  if (input.address){
    addr = bcash.Address(bitcore.Address(input.address), bcash.Networks.get(this.node.getNetworkName())).toCashAddress();
  }
  transformed.addr = addr;
...
TxController.prototype.transformOutput = function(options, output, index) {
  ...
  if (output.address) {
    var addrs = bcash.Address(bitcore.Address(output.address), bcash.Networks.get(this.node.getNetworkName()))
    transformed.scriptPubKey.addresses = [addrs.toCashAddress()];
    transformed.scriptPubKey.type = addrs.type;
  }
  return transformed;
};
...
TxController.prototype.transformInvTransaction = function(transaction) {
  ...
  for (var i = 0; i < transaction.outputs.length; i++) {
    ...
    if (output.script) {
      ...
      if (address) {
        var obj = {};
        obj[new bcash.Address(address, bcash.Networks.get(self.node.getNetworkName())).toCashAddress()] = output.satoshis;
        vout.push(obj);
      }
    }
  }
